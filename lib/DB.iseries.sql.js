...
function multiCodeLOV(code, jsh, lov) {
  return 'SELECT ' + jsh.map.code_val + ',' + jsh.map.code_txt + ' FROM ' + jsh.map[code] + '_' + lov[code] + ' WHERE (' + jsh.map.code_end_date + ' IS NULL OR ' + jsh.map.code_end_date + '>CURRENT_TIMESTAMP)';
}
