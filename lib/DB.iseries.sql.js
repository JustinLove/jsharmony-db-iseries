...
DBsql.prototype.getLOV = function (jsh, fname, lov, datalockqueries, param_datalocks, options) {
  ...
  else if ('code' in lov) { sql = 'SELECT "' + jsh.map.code_val + '","' + jsh.map.code_txt + '" FROM '+(lov.schema?lov.schema+'.':'')+ jsh.map.code + '_' + lov['code'] + ' WHERE ("' + jsh.map.code_end_date + '" IS NULL OR "' + jsh.map.code_end_date + '">CURRENT_TIMESTAMP) ORDER BY "' + jsh.map.code_seq + '","' + jsh.map.code_txt + '"'; }
  else if ('code2' in lov) { sql = 'SELECT "' + jsh.map.code_val + '1" AS "' + jsh.map.code_parent + '","' + jsh.map.code_val + '2" as "' + jsh.map.code_val + '","' + jsh.map.code_txt + '" FROM '+(lov.schema?lov.schema+'.':'')+ jsh.map.code2 + '_' + lov['code2'] + ' WHERE ("' + jsh.map.code_end_date + '" IS NULL OR "' + jsh.map.code_end_date + '">CURRENT_TIMESTAMP) ORDER BY "' + jsh.map.code_seq + '","' + jsh.map.code_txt + '"'; }
  else if ('code_sys' in lov) { sql = 'SELECT "' + jsh.map.code_val + '","' + jsh.map.code_txt + '" FROM '+(lov.schema?lov.schema+'.':'')+ jsh.map.code_sys + '_' + lov['code_sys'] + ' WHERE ("' + jsh.map.code_end_date + '" IS NULL OR "' + jsh.map.code_end_date + '">CURRENT_TIMESTAMP) ORDER BY "' + jsh.map.code_seq + '","' + jsh.map.code_txt + '"'; }
  ...
  else if ('code_app' in lov) { sql = 'SELECT "' + jsh.map.code_val + '","' + jsh.map.code_txt + '" FROM '+(lov.schema?lov.schema+'.':'')+ jsh.map.code_app + '_' + lov['code_app'] + ' WHERE ("' + jsh.map.code_end_date + '" IS NULL OR "' + jsh.map.code_end_date + '">CURRENT_TIMESTAMP) ORDER BY "' + jsh.map.code_seq + '","' + jsh.map.code_txt + '"'; }
  ...
  // Note that AS is not quoted here - jsharmony-db passes a datalockquery with an unquoted symbol, so we we have to the same in order for the case to match
  _.each(param_datalocks, function (param_datalock) {
    sql = addDataLockSQL(sql, "SELECT " + XtoDB(jsh, param_datalock.field, '@' + param_datalock.pname) + " AS " + param_datalock.pname + " FROM SYSIBM.SYSDUMMY1", param_datalock.datalockquery) ;
  });
