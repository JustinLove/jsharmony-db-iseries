...
DBmeta.prototype.metaIncludeJoin = function(schema_column, table_column, metadata_filter) {
  ...
  if (!metadata_filter || !metadata_filter.length || _.includes(metadata_filter, '%.%')) {
...
DBmeta.prototype.metaInclude = function(schema_column, table_column, metadata_filter) {
  ...
  if(!metadata_filter || !metadata_filter.length){
    return "(0=1)";
  }
...
DBmeta.prototype.getTables = function(table, options, callback){
  ...
  var sql = "SELECT T.TABLE_SCHEMA \"schema_name\", T.TABLE_NAME \"table_name\", T.LONG_COMMENT \"description\", T.TABLE_TYPE \"table_type\" \
    FROM QSYS2.SYSTABLES T " + _this.metaIncludeJoin('T.TABLE_SCHEMA', 'T.TABLE_NAME', _this.db.dbconfig.options.metadata_filter) +" \
    WHERE T.TABLE_TYPE IN ('T', 'V') AND T.FILE_TYPE = 'D' \
      ";
  if(!_this.db.dbconfig.options.metadata_filter || _this.db.dbconfig.options.metadata_filter.length < 1){
    sql += ' AND 0=1 ';
  }
  if(table){
...
FROM QSYS2.SYSCOLUMNS C " + _this.metaIncludeJoin('C.TABLE_SCHEMA', 'C.TABLE_NAME', _this.db.dbconfig.options.metadata_filter) +" \
WHERE C.TABLE_SCHEMA = COALESCE(@schema_name, C.TABLE_SCHEMA) \
   AND C.TABLE_NAME = COALESCE(@table_name, C.TABLE_NAME) \
   AND " + _this.metaInclude('C.TABLE_SCHEMA', 'C.TABLE_NAME', _this.db.dbconfig.options.metadata_filter) + " \
...
FROM SYSIBM.SQLFOREIGNKEYS " + _this.metaIncludeJoin('FKTABLE_SCHEM', 'FKTABLE_NAME', _this.db.dbconfig.options.metadata_filter) +"\
